...
function deleteInfo(cusId) {
    var con = fs.readFileSync("./nodeModule/customerInfo.json", "utf-8");
    con = (con !== "") ? JSON.parse(con) : [];
    var flag=false;
...
function getData(cusId) {
    var con = fs.readFileSync("./nodeModule/customerInfo.json", "utf8");
    con = (con !== "") ? JSON.parse(con) : [];//如果获取的内容为空,说明JSON文件中没有任何的客户信息,为了方便解析和循环,对应这种情况我们让con等于一个空数组即可
    var res=null;
    for(var i=0;i<con.length;i++){
        //通过传递进来的客户ID把对应数据获取到
        var cur=con[i];
