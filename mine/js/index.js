...
~function () {
    ...
    window.setTimeout(function () {
        ...
        myAudio.addEventListener("canplay",function () {
            ...
            //console.log(audioBox.style.display);
...
//移动端是可以使用click事件的,但是click事件存在300ms延迟(当第一次点击的时候,浏览器需要等待300ms,看是否存在第二次触发,如果存在第二次触发,属于双击;不存在才是单击)

//解决方案一:使用click,把300ms延迟解决,使用fastClick.js


//解决方案二:不用click,使用touch事件模型模拟出单击的效果
    //移动端的事件和pc端的事件是不一样的,我们所有的操作依靠的是手指
    //点击(tap),单击(singleTap),双击(doubleTap),长摁(longTap),滑动(swipe),上滑(swipeUp),下滑(swipeDown),左滑(swipeLeft),右滑(swipeRight)-->"单手指操作":我们使用touch事件模型把上述操作模拟出来

//touchstart,touchmove,touchend,touchcancle


//点击 && 滑动:手指结束位置和手指的起始位置的偏移距离,小于30像素算作点击点击,大于30像素算作滑动
//单击 && 双击:第一次触发touchstart之后,在300ms内是否触发了第二次
//长摁:在750ms之内是否离开屏幕,没有离开属于长摁
//滑动:
...
//移动端专有的事件库
//1)Zepto.js(只是用来处理移动端的,移动端小型的jQuery库),在这个库中提供了移动端的专用事件以及提供了css3动画的支持
        
    //他与jQuery的区别
        //兼容性
        //选择器,而他只支持十几种,小型轻量级
        //他提供了移动端专用的事件和css3动画的支持
/*$("#div1").tap(function () {

});*/

//2)touch.js(百度云touch手势事件库)
